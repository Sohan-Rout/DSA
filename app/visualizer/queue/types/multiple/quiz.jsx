...
const questions = [
    {
      question: "What is the fundamental characteristic of a multiple queue structure?",
      options: [
        "A single queue with multiple priority levels",
        "Several independent FIFO queues managed together",
        "A tree-like structure of interconnected queues",
        "A queue that automatically sorts elements"
      ],
      correctAnswer: 1,
      explanation: "Multiple queues consist of several distinct FIFO queues managed collectively."
    },
    {
      question: "In a multiple queue system, what does enqueue(item, queueNum) do?",
      options: [
        "Adds an item to all queues simultaneously",
        "Inserts an item into a specific numbered queue",
        "Randomly distributes items across queues",
        "Replaces the oldest item in any queue"
      ],
      correctAnswer: 1,
      explanation: "Items are added to the explicitly specified queue number while maintaining FIFO order within that queue."
    },
    {
      question: "What is the time complexity of dequeue(queueNum) in a well-implemented multiple queue?",
      options: ["O(n)", "O(1)", "O(log k)", "O(k)"],
      correctAnswer: 1,
      explanation: "Dequeue from a specific queue is O(1), same as a standard queue."
    },
    {
      question: "Which implementation uses dynamic memory allocation for multiple queues?",
      options: [
        "Fixed array of arrays",
        "Array of linked lists",
        "Single circular buffer",
        "Stack-based queues"
      ],
      correctAnswer: 1,
      explanation: "An array of linked lists allows each queue to grow independently."
    },
    {
      question: "What would multiDequeue() return in a priority-based multiple queue with Queue0 (high priority) = [A], Queue1 = [B,C]?",
      options: ["A", "B", "C", "null"],
      correctAnswer: 0,
      explanation: "Priority implementations dequeue from the highest-priority non-empty queue first."
    },
    {
      question: "Which real-world system most likely uses multiple queues?",
      options: [
      ...
      correctAnswer: 0,
      explanation: "OS schedulers often use multiple priority queues for processes."
    },
    {
      question: "What's a key advantage of multiple queues over a single priority queue?",
      options: [
        "Constant-time insertion for all priorities",
        "Strict FIFO maintenance within categories",
        "Automatic load balancing",
        "Smaller memory footprint"
      ],
      correctAnswer: 1,
      explanation: "Multiple queues preserve FIFO ordering within each priority/category."
    },
    {
      question: "How would you check if Queue2 is empty in a 3-queue system?",
      options: [
        "isEmpty(2)",
        "isQueueEmpty(2)",
        "queue[2].isEmpty()",
      ...
      correctAnswer: 3,
      explanation: "Implementation determines the exact syntax, but all options represent valid approaches."
    },
    {
      question: "What happens in a bounded multi-queue when enqueue() is called on a full queue?",
      options: [
        "Items overflow to next queue",
        "Operation fails/throws error",
        "Oldest item is automatically evicted",
        "Queue capacity doubles"
      ],
      correctAnswer: 1,
      explanation: "Bounded queues typically reject additions when full (though some implementations may use other policies)."
    },
    {
      question: "Which operation would have O(k) complexity where k is number of queues?",
      options: [
        "enqueue(item, queueNum)",
        "dequeue(queueNum)",
        "multiDequeue()",
        "peek(queueNum)"
      ],
      correctAnswer: 2,
      explanation: "multiDequeue() may need to check up to k queues when using priority rules."
    }
];
