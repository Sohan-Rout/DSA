...
const DiameterQuiz = () => {
  const questions = [
    {
      question: "What does the diameter of a tree measure?",
      options: [
    ...
    },
    {
      ...
      options: [
        "Its own height only",
        "The height of its left subtree plus the height of its right subtree",
        "The number of its direct children",
    ...
    },
    {
      ...
      correctAnswer: 1,
      explanation: "Post-order guarantees both children's heights are already known before a node computes its own height and path-through value, so nothing is recomputed."
    },
    {
      question: "What is the time complexity of the single-pass diameter algorithm?",
      options: ["O(1)", "O(log n)", "O(n)", "O(n²)"],
      correctAnswer: 2,
      explanation: "Every node is visited exactly once to compute its height and update the running maximum, giving O(n) overall."
    }
  ];
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("the definition of tree diameter");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("how the path-through value is calculated at a node");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("why the diameter path doesn't have to pass through the root");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("why post-order traversal avoids recomputation");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("time complexity of the algorithm");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered Tree Diameter!";
  };
