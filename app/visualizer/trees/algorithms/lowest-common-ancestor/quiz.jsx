...
const LcaQuiz = () => {
  const questions = [
    {
      question: "What does the Lowest Common Ancestor of nodes A and B mean?",
      options: [
        "The node closest to A in the whole tree",
        "The deepest node that is an ancestor of both A and B",
        "The average of A and B's values",
    ...
    },
    {
      ...
      options: [
        ...
        "The LCA must be in the left subtree, so the right subtree can be skipped entirely",
    ...
    },
    {
      ...
      correctAnswer: 1,
      explanation: "A BST's ordering lets a comparison rule out an entire subtree without looking inside it. A general binary tree has no such guarantee, so both children must be searched to find A and B."
    },
    {
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("the definition of Lowest Common Ancestor");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("how BST ordering rules out a subtree");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("recognizing the split point");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("why general trees can't skip a subtree");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("worst-case time complexity");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered Lowest Common Ancestor!";
  };
