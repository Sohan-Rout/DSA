...
const IsomorphismQuiz = () => {
  const questions = [
    {
      question: "What does it mean for two binary trees to be isomorphic?",
      options: [
        "They have the exact same number of nodes",
        "One can be transformed into the other by swapping left/right children at some nodes, with matching values and structure",
    ...
    },
    {
      ...
      correctAnswer: 2,
      explanation: "If one side has a node and the other doesn't at the same comparison point, the structures don't match, so the trees can't be isomorphic."
    },
    {
      ...
      correctAnswer: 1,
      explanation: "Trying both the straight and flipped pairings is exactly what allows a swap at this node without breaking the match."
    },
    {
      question: "How does an isomorphism check differ from a plain structural-equality check?",
      options: [
        "It doesn't check values, only shape",
        "It also attempts the flipped (swapped-children) pairing before declaring a mismatch",
        "It ignores leaf nodes",
        "It only works on Binary Search Trees"
      ],
      correctAnswer: 1,
      explanation: "Structural equality only tries the straight pairing. Isomorphism adds the flipped attempt, making it forgiving of left/right swaps."
    },
    {
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("the definition of tree isomorphism");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("handling a null-vs-node mismatch");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("the straight vs flipped comparison");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("how isomorphism differs from structural equality");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("worst-case complexity of the naive check");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered Tree Isomorphism!";
  };
...
export default IsomorphismQuiz;
