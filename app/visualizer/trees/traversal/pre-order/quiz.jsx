...
const PreOrderQuiz = () => {
  const questions = [
    {
      question: "What is the visiting order for pre-order traversal?",
      options: ["Left, Root, Right", "Root, Left, Right", "Left, Right, Root", "Right, Root, Left"],
      correctAnswer: 1,
      explanation: "Pre-order visits the root first, then recursively traverses the left subtree, then the right subtree."
    },
    {
      question: "For the tree rooted at 8 with left child 3 (children 1 and 6) and right child 10, what is the pre-order sequence?",
      options: ["[1, 3, 6, 8, 10]", "[8, 3, 1, 6, 10]", "[1, 6, 3, 10, 8]", "[8, 10, 3, 1, 6]"],
      correctAnswer: 1,
      explanation: "Visit 8 first, then descend fully into its left subtree (3, then 1, then 6), then its right subtree (10)."
    },
    {
      question: "Why is pre-order the traversal typically used to serialize (save) a tree so it can be rebuilt later?",
      options: [
        "It's the fastest traversal",
        "It uses the least memory of any traversal",
        "The root is always recorded before its children, so the structure can be rebuilt top-down while reading the sequence",
        "It only works on balanced trees"
      ],
      correctAnswer: 2,
      explanation: "Since every subtree's root appears before its children in the output, a reader can reconstruct the tree top-down as it consumes the sequence."
    },
    {
    ...
    {
      question: "What is the space complexity of a recursive pre-order traversal, in terms of tree height h?",
      options: ["O(1)", "O(h)", "O(n)", "O(n log n)"],
      correctAnswer: 1,
      explanation: "The only extra space used is the recursion call stack, which grows to at most the height of the tree, h."
    }
  ];
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("the Root-Left-Right visiting order");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("tracing pre-order on an example tree");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("why pre-order is used for serialization");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("the link between pre-order and prefix notation");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("space complexity of recursive traversal");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered pre-order traversal!";
  };
