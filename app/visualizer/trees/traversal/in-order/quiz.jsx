...
const InOrderQuiz = () => {
  const questions = [
    {
      question: "What is the visiting order for in-order traversal?",
      options: ["Root, Left, Right", "Left, Root, Right", "Left, Right, Root", "Right, Root, Left"],
      correctAnswer: 1,
      explanation: "In-order visits the left subtree first, then the root, then the right subtree."
    },
    {
      question: "For the tree rooted at 8 with left child 3 (children 1 and 6) and right child 10, what is the in-order sequence?",
      options: ["[8, 3, 1, 6, 10]", "[1, 3, 6, 8, 10]", "[1, 6, 3, 10, 8]", "[10, 8, 6, 3, 1]"],
      correctAnswer: 1,
      explanation: "Fully descend left before visiting a node: 1, then 3, then 6 (left subtree done), then 8, then 10."
    },
    {
      question: "What special property does in-order traversal have when run on a Binary Search Tree?",
      options: [
        "It visits nodes in random order",
        "It produces the values in strictly ascending sorted order",
        "It only visits leaf nodes",
        "It visits the root twice"
      ],
      correctAnswer: 1,
      explanation: "Because every BST node's left subtree is smaller and right subtree is larger, visiting Left-Root-Right at every level naturally yields sorted output."
    },
    {
      ...
      correctAnswer: 1,
      explanation: "If the in-order output is ever out of order, some node breaks the left-smaller/right-larger rule, so the tree can't be a valid BST."
    },
    {
      question: "What is the space complexity of a recursive in-order traversal, in terms of tree height h?",
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("the Left-Root-Right visiting order");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("tracing in-order on an example tree");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("why in-order produces sorted output on a BST");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("using in-order to validate a BST");
    }
    ...
    return weakAreas.length > 0
      ...
      : "Perfect! You've mastered in-order traversal!";
  };
