...
const PostOrderQuiz = () => {
  const questions = [
    {
      question: "What is the visiting order for post-order traversal?",
      options: ["Root, Left, Right", "Left, Root, Right", "Left, Right, Root", "Right, Left, Root"],
      correctAnswer: 2,
      explanation: "Post-order visits the left subtree, then the right subtree, and visits the root last of all."
    },
    {
      question: "For the tree rooted at 8 with left child 3 (children 1 and 6) and right child 10, what is the post-order sequence?",
      options: ["[8, 3, 1, 6, 10]", "[1, 3, 6, 8, 10]", "[1, 6, 3, 10, 8]", "[10, 8, 6, 3, 1]"],
      correctAnswer: 2,
      explanation: "Fully finish the left subtree (1, then 6, then 3), then the right subtree (10), and only then visit the root, 8, last."
    },
    {
      question: "Why is post-order the traversal typically used to delete or free every node in a tree?",
      options: [
        "It's the fastest traversal algorithm",
        "A node's children are visited (and freed) before the node itself, so you never lose your only reference to a subtree",
        "It uses no recursion, so it avoids stack overflow",
    ...
    {
      question: "What is the space complexity of a recursive post-order traversal, in terms of tree height h?",
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("the Left-Right-Root visiting order");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("tracing post-order on an example tree");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("why post-order is used for tree deletion");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("the link between post-order and postfix notation");
    }
    ...
    return weakAreas.length > 0
      ...
      : "Perfect! You've mastered post-order traversal!";
  };
