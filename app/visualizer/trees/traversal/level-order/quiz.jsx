...
const LevelOrderQuiz = () => {
  const questions = [
    {
      question: "What is another common name for level-order traversal?",
      options: ["Depth-First Search (DFS)", "Breadth-First Search (BFS)", "In-order Search", "Binary Search"],
      correctAnswer: 1,
      explanation: "Level-order traversal visits nodes level by level, which is exactly what Breadth-First Search does on a tree."
    },
    {
      question: "What data structure does level-order traversal use to keep track of which node to visit next?",
      options: ["A stack", "A queue", "A hash map", "A priority queue"],
      correctAnswer: 1,
      explanation: "A FIFO queue guarantees nodes come back out in the same order they were discovered, which is what keeps entire levels together."
    },
    {
      question: "For the tree rooted at 8 with left child 3 (children 1 and 6) and right child 10, what is the level-order sequence?",
      options: ["[1, 3, 6, 8, 10]", "[8, 3, 1, 6, 10]", "[8, 3, 10, 1, 6]", "[1, 6, 3, 10, 8]"],
      correctAnswer: 2,
      explanation: "Visit level 0 (8), then all of level 1 left to right (3, 10), then all of level 2 left to right (1, 6)."
    },
    {
      question: "Why is level-order traversal the natural choice for finding the shortest path or minimum depth in an unweighted tree?",
      options: [
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("that level-order is the same thing as BFS");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("why a queue (not a stack) drives level-order");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("tracing level-order on an example tree");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("why BFS finds shortest paths in unweighted trees");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("space complexity in terms of tree width");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered level-order traversal!";
  };
