...
const BstSearchingQuiz = () => {
  const questions = [
    {
      question: "When searching a BST, what does it mean if the target is smaller than the current node?",
      options: ["Search the right subtree", "Search the left subtree", "The value isn't in the tree", "Stop searching"],
      correctAnswer: 1,
      explanation: "The BST property guarantees smaller values live in the left subtree, so the search continues there."
    },
    {
      question: "How does the search know a value isn't in the tree?",
      options: [
        "It checks every node in the tree first",
        "It reaches a null pointer while walking down",
    ...
    },
    {
      ...
      options: [
        "BSTs store fewer elements",
        "Each comparison eliminates an entire subtree instead of just one element",
        "BSTs use less memory",
        "It isn't actually faster"
      ],
      correctAnswer: 1,
      explanation: "Because of the ordering property, one comparison lets you discard half the remaining nodes, giving O(log n) instead of the O(n) a linear scan requires."
    },
    {
    ...
    },
    {
      ...
      options: [
        "They're actually different algorithms",
        "All three follow the same root-to-leaf comparison walk, so they're all bounded by the tree's height",
        "Coincidence",
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("comparison direction rules");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("how a 'not found' result is detected");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("why BST search beats a linear scan");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("best-case complexity");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("why search/insert/delete share the same complexity");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered BST searching!";
  };
