...
// Walks the search path while also recording, at each step, every value in
// the subtree that gets ruled out by that comparison.
const searchWithElimination = (root, value) => {
  const path = [];
  ...
  while (cur) {
    ...
    if (value === cur.value) {
      ...
      return { found: true, path, eliminatedPerStep };
    }
    if (value < cur.value) {
  ...
  return { found: false, path, eliminatedPerStep };
};
...
const BstSearchingVisualizer = () => {
  ...
  const handleSearch = () => {
    ...
    const { found, path, eliminatedPerStep } = searchWithElimination(root, value);
    setBusy(true);
    setHighlightPath([]);
    setDimmedValues([]);
    setFoundValue(null);
    ...
    const revealStep = () => {
      ...
      setMessage(`Comparing ${value} with ${current}...`);
      step++;
        ...
        <div className="bg-white dark:bg-neutral-950 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
          ...
          <div className="min-h-60 flex justify-center overflow-auto py-4 rounded-lg bg-[radial-gradient(circle,var(--color-gray-200)_1px,transparent_1px)] dark:bg-[radial-gradient(circle,var(--color-neutral-800)_1px,transparent_1px)] bg-size-[16px_16px]">
            {nodes.length > 0 ? (
              ...
              >
                <defs>
                  <linearGradient id="bs-internal-grad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#60a5fa" />
                    <stop offset="100%" stopColor="#2563eb" />
                  </linearGradient>
                  <linearGradient id="bs-leaf-grad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#34d399" />
                    <stop offset="100%" stopColor="#059669" />
                  </linearGradient>
                  <filter id="bs-node-shadow" x="-50%" y="-50%" width="200%" height="200%">
                ...
                {nodes.map((node, i) => {
                  ...
                  const isNotFoundEnd = node.value === notFoundValue;
                  const isDimmed = dimmedValues.includes(node.value) && !isOnPath && !isFound && !isNotFoundEnd;
                  return (
                    ...
                    >
                      ...
                      {isOnPath && !isFound && !isNotFoundEnd && (
                        <circle
                          cx={node.x}
                          cy={node.y}
                          r={NODE_RADIUS + 4}
                      ...
                      <circle
                        ...
                        cy={node.y}
                        r={NODE_RADIUS}
                        fill={node.isLeaf ? "url(#bs-leaf-grad)" : "url(#bs-internal-grad)"}
                        stroke={node.isLeaf ? "#059669" : "#1d4ed8"}
                        strokeWidth="1.5"
                        filter="url(#bs-node-shadow)"
                      />
                      <text
                        x={node.x}
                        y={node.y + 5}
...
export default BstSearchingVisualizer;
