...
const BstDeletionQuiz = () => {
  const questions = [
    {
      ...
      correctAnswer: 1,
      explanation: "A leaf has no children, so deleting it is trivial: just detach it from its parent."
    },
    {
      question: "How do you delete a node that has exactly one child?",
      options: [
      ...
      correctAnswer: 1,
      explanation: "With only one child, the parent can simply point directly to that child, splicing the deleted node out."
    },
    {
      question: "What is the in-order successor of a node used for when deleting a node with two children?",
      options: [
        "It becomes the new root",
        "Its value replaces the deleted node's value, then the successor itself is removed",
        "It is deleted instead of the target node",
        "It is ignored"
      ],
      correctAnswer: 1,
      explanation: "The in-order successor (smallest value in the right subtree) is copied into the node being deleted, preserving BST order, and then the successor's original position is removed."
    },
    {
      question: "Where is the in-order successor of a node always located?",
      options: [
      ...
      correctAnswer: 1,
      explanation: "The in-order successor is the smallest value greater than the node, found by going right once then left as far as possible."
    },
    {
      question: "Why does deleting the in-order successor never require handling the two-children case again?",
      options: [
        "It's always the root",
        "The successor is guaranteed to have no left child, reducing it to Case 1 or Case 2",
        "Successors are always leaves",
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("deleting a leaf node");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("deleting a node with one child");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("the two-children deletion case");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("locating the in-order successor");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("why successor removal never recurses into another two-children case");
    }
