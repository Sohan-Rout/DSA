...
const BstInsertionVisualizer = () => {
  ...
  const handleInsert = () => {
    ...
    setRoot((prev) => {
      ...
      return insertNode(cloned, value, path);
    });

    lastInsertedRef.current = value;
    setMessage(
      path.length > 0
        ? `Compared against ${path.join(" → ")}, then inserted ${value}`
    ...
    highlightTimeoutRef.current = setTimeout(() => setHighlightPath([]), 1100);
  };
        ...
        <div className="bg-white dark:bg-neutral-950 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
          ...
          <div className="min-h-60 flex justify-center overflow-auto py-4 rounded-lg bg-[radial-gradient(circle,var(--color-gray-200)_1px,transparent_1px)] dark:bg-[radial-gradient(circle,var(--color-neutral-800)_1px,transparent_1px)] bg-size-[16px_16px]">
            {nodes.length > 0 ? (
              ...
              >
                <defs>
                  <linearGradient id="bi-internal-grad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#60a5fa" />
                    <stop offset="100%" stopColor="#2563eb" />
                  </linearGradient>
                  <linearGradient id="bi-leaf-grad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#34d399" />
                    <stop offset="100%" stopColor="#059669" />
                  </linearGradient>
                  <filter id="bi-node-shadow" x="-50%" y="-50%" width="200%" height="200%">
                  ...
                  return (
                    <g key={i} ref={animateDropIn}>
                      ...
                      <circle
                        ...
                        cy={node.y}
                        r={NODE_RADIUS}
                        fill={node.isLeaf ? "url(#bi-leaf-grad)" : "url(#bi-internal-grad)"}
                        stroke={node.isLeaf ? "#059669" : "#1d4ed8"}
                        strokeWidth="1.5"
                        filter="url(#bi-node-shadow)"
                      />
                      <text
                        x={node.x}
                        y={node.y + 5}
...
export default BstInsertionVisualizer;
