...
const BstInsertionQuiz = () => {
  const questions = [
    {
      question: "In a BST, where do you go if the value you're inserting is greater than the current node?",
      options: ["Left child", "Right child", "Stop and replace it", "Insert at the root"],
      correctAnswer: 1,
      explanation: "BSTs keep larger values in the right subtree, so a greater value means you continue into the right child."
    },
    {
    ...
    },
    {
      ...
      options: [
        "Because BSTs are stored as arrays",
        "Because the new node is simply attached at the empty spot found during the search",
        "Because nodes are moved automatically by a background process",
    ...
    },
    {
      ...
      correctAnswer: 1,
      explanation: "In a balanced tree, insertion follows a root-to-leaf path of length proportional to the height, which is O(log n) when balanced."
    },
    {
      question: "What happens to insertion's time complexity if you insert already-sorted data into a plain BST?",
      options: [
        "It stays O(log n)",
        "It improves to O(1)",
        "It degrades to O(n) since the tree becomes a skewed linked list",
        "Insertion fails"
      ],
      correctAnswer: 2,
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("BST comparison direction rules");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("where new nodes land");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("why insertion doesn't move existing nodes");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("time complexity in a balanced tree");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("worst-case insertion order");
    }
