...
const AvlQuiz = () => {
  const questions = [
    {
      question: "What extra rule does an AVL tree enforce beyond the normal BST ordering?",
      options: [
        "Every node must have exactly two children",
        "The heights of a node's left and right subtrees may differ by at most 1",
    ...
    {
      question: "Which rotation fixes a Left-Right (zig-zag) imbalance?",
      options: [
        "A single right rotation only",
        "A single left rotation only",
        "A left rotation on the left child, followed by a right rotation on the node",
        "A right rotation on the left child, followed by a left rotation on the node"
      ],
      correctAnswer: 2,
      explanation: "Left-Right is a zig-zag case: first rotate the left child left to turn it into a straight Left-Left shape, then rotate the node right."
    },
    {
    ...
    },
    {
      ...
      options: [
        "AVL trees use less memory per node",
        "Plain BSTs can degenerate into a skewed, linked-list-like shape with O(n) operations; AVL trees guarantee O(log n) worst-case by rebalancing",
        "AVL trees don't need to maintain BST ordering",
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("the AVL balance condition");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("when rotations trigger");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("the Left-Right double rotation");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("worst-case height guarantees");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("why self-balancing trees exist");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered AVL tree balancing!";
  };
