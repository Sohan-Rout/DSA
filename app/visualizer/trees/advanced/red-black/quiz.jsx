...
const RedBlackQuiz = () => {
  const questions = [
    ...
    },
    {
      ...
      correctAnswer: 1,
      explanation: "Coloring the new node black would immediately break the equal-black-height property on that path. Red only risks breaking the 'no red parent' rule, which is easier to fix."
    },
    {
      question: "When fixing a red-red violation, if the new node's uncle is also red, what does the fixup do?",
      options: [
        "Rotate immediately",
    ...
    },
    {
      ...
      options: [
        "Red-Black Trees don't use rotations at all",
        "Red-Black Trees enforce a looser balance condition (color rules, not strict height), so recoloring alone often resolves a violation without any rotation",
        "AVL trees never need rotations",
        "Red-Black Trees are always smaller"
      ],
      correctAnswer: 1,
      explanation: "AVL's strict height-balance requirement can cascade rotations back up the tree, while Red-Black's looser color-based rules mean many violations are resolved by recoloring alone, needing at most one or two rotations."
    },
    {
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("the root-must-be-black rule");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("why new nodes are colored red");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("the red-uncle recoloring case");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("why Red-Black Trees need fewer rotations than AVL");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("worst-case time complexity guarantees");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered Red-Black Trees!";
  };
