...
const SegmentTreeQuiz = () => {
  const questions = [
    {
      question: "What problem does a segment tree solve better than a prefix-sum array?",
      options: [
        "It uses less memory than any other structure",
        "It supports both range queries and point updates in O(log n), while a prefix-sum array needs O(n) to update",
        "It can only answer queries, not updates",
        "It sorts the array automatically"
      ],
      correctAnswer: 1,
      explanation: "A prefix-sum array answers range queries in O(1) but a single update forces recomputing every prefix after it (O(n)). A segment tree keeps both operations at O(log n)."
    },
    {
      question: "What does a segment tree node store?",
      options: [
        "A single array element only",
        "The combined result (e.g. sum, min, max) of an associative operation over the contiguous range it represents",
        "A random value",
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("why segment trees beat prefix-sum arrays");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("what a segment tree node stores");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("range query decomposition");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("what happens during a point update");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("build time complexity");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered segment trees!";
  };
