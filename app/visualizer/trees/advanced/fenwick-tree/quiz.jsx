...
const FenwickQuiz = () => {
  const questions = [
    {
      question: "What determines the size of the range that BIT index i is responsible for?",
      options: [
        "The value stored at BIT[i]",
        "The value of i's lowest set bit (lowbit(i))",
        "It's always exactly 1 element",
    ...
    },
    {
      ...
      options: ["i += lowbit(i)", "i -= lowbit(i)", "i = i + 1", "i = 2 * i"],
      correctAnswer: 1,
      explanation: "Subtracting the lowbit moves down to the next chunk of the prefix that still needs to be accumulated, until the whole prefix has been summed."
    },
    {
      question: "How is a range sum from l to r computed using a Fenwick Tree?",
      options: [
        "By visiting every element from l to r individually",
        "As prefixSum(r) - prefixSum(l - 1)",
        "Fenwick trees can't compute range sums, only prefix sums",
        "By rebuilding the tree with a new starting index"
      ],
      correctAnswer: 1,
      explanation: "Since the BIT only directly supports prefix sums, a range sum is derived by subtracting the prefix just before the range from the prefix ending at the range."
    },
    {
      question: "Why can't a Fenwick Tree be used for range-minimum or range-maximum queries the way it can for range sums?",
      options: [
        "It technically can, with no changes needed",
        "Because the range[l,r] = prefix(r) - prefix(l-1) trick requires an invertible operation, and min/max have no inverse",
        "Because minimum and maximum are always O(n) operations",
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("how lowbit determines a BIT index's range");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("index movement during a point update");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("index movement during a prefix sum query");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("how range sums are derived from prefix sums");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("why Fenwick trees are limited to invertible operations");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered Fenwick Trees!";
  };
