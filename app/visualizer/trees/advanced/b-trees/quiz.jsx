...
const BTreeQuiz = () => {
  const questions = [
    ...
    },
    {
      ...
      options: [
        "To make the tree look more visually interesting",
        "To minimize the number of disk reads, since each node/block read is expensive regardless of how much of it is used",
        "To use less memory than a binary tree",
    ...
    },
    {
      ...
      options: [
        "A node reaching exactly 2t-1 (3) keys, right before a new key would be inserted into or through it",
        "Every single insertion, regardless of node size",
        "Only when the tree becomes taller than 5 levels",
        "Splits never happen during insertion"
      ],
      correctAnswer: 0,
      explanation: "The proactive-split strategy splits any full node (2t-1 keys) it's about to insert into or descend through, before the insertion happens."
    },
    {
      question: "What happens to the median key of a node when it splits?",
      options: [
        "It's deleted",
        "It moves up into the parent node, separating the two halves that used to be one node",
        "It stays in the left half",
        "It's duplicated into both halves"
      ],
      correctAnswer: 1,
      explanation: "The median key is promoted to the parent, becoming the new boundary between the left half (keys before it) and the right half (keys after it)."
    },
    {
      question: "Why does a B-Tree never become unbalanced the way a plain BST can?",
      options: [
        "It doesn't use comparisons",
        "It only grows in height by splitting the root, so every leaf always ends up at exactly the same depth",
        "It has no leaves",
        "It rebalances using AVL-style rotations"
      ],
      correctAnswer: 1,
      explanation: "A B-Tree grows upward from the root (via root splits) rather than downward from arbitrary leaves, which guarantees every leaf stays at the same depth."
    }
  ];
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("the keys-to-children relationship");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("why B-Trees favor wide branching for disk I/O");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("when a node split is triggered");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("what happens to the median key during a split");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("why every leaf stays at the same depth");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered B-Trees!";
  };
