...
const PrefixTreeQuiz = () => {
  const questions = [
    {
      question: "In a trie, what does a single edge between two nodes typically represent?",
      options: ["An entire word", "A single character", "A comparison result", "A pointer to a random node"],
      correctAnswer: 1,
      explanation: "Unlike a BST where a node stores a whole key, a trie's edges each represent one character, and paths from the root spell out prefixes."
    },
    {
      question: "Why do 'cat' and 'car' share part of the same path when both are inserted into a trie?",
      options: [
        "It's a coincidence specific to those two words",
        "Because they share the same prefix 'ca', and a trie reuses existing edges for shared prefixes instead of duplicating them",
        "Tries always merge every word into one path",
    ...
    },
    {
      ...
      correctAnswer: 1,
      explanation: "A path existing means the string is at least a prefix of some stored word. Only the end-of-word flag confirms that exact string was itself inserted as a complete word."
    },
    {
      question: "Why is a trie better suited than a hash set for 'autocomplete' style prefix queries?",
      options: [
        "Hash sets can't store strings",
        "A trie's structure means the entire subtree under a matching prefix node already contains exactly the words that share that prefix",
        "Tries are always faster for exact-match lookups",
    ...
    },
    {
      ...
      options: ["O(1)", "O(L)", "O(n), where n is the number of words already stored", "O(L log n)"],
      correctAnswer: 1,
      explanation: "Both operations walk exactly one node per character of the word, so the cost depends only on the word's length L, not on how many words are already in the trie."
    }
  ];
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("what a trie edge represents");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("how shared prefixes reuse paths");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("the end-of-word flag vs. a matching path");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("why tries suit prefix/autocomplete queries");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("time complexity in terms of word length");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered tries!";
  };
