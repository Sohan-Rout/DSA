...
const SyntaxTreeQuiz = () => {
  const questions = [
    {
      question: "In a syntax tree for an arithmetic expression, what do the internal nodes and leaves represent?",
      options: [
        "Internal nodes are numbers, leaves are operators",
        "Internal nodes are operators, leaves are operand values",
        "Internal nodes and leaves are both operators",
    ...
    },
    {
      ...
      correctAnswer: 1,
      explanation: "Post-order traversal visits both children before the node itself, which is precisely the order evaluation needs: operands ready before the operator combines them."
    },
    {
      question: "What does reading a syntax tree with a pre-order traversal produce?",
      options: [
        "Infix notation",
        "Prefix notation",
        "Postfix notation",
    ...
    },
    {
      ...
      options: [
        "They actually do need parentheses too",
        "The fixed traversal order itself fully determines how operators and operands pair up, so no grouping symbols are needed",
        "Because they only support single-digit numbers",
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("what internal nodes and leaves represent");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("how recursive-descent parsing handles precedence");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("why post-order traversal matches evaluation order");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("what pre-order traversal produces");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("why prefix/postfix notation don't need parentheses");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered Syntax Trees!";
  };
