...
const BinaryTreeTypesQuiz = () => {
  const questions = [
    {
      question: "What defines a Full Binary Tree?",
      options: [
        "Every level is completely filled",
        "Every node has either zero or exactly two children",
        "The tree has the minimum possible height",
    ...
    },
    {
      ...
      options: ["A hash table", "A linked list", "A heap", "A graph"],
      correctAnswer: 1,
      explanation: "Since every node has only one child, a degenerate tree is structurally just a linked list, giving it the same Θ(n) height and worst-case performance."
    },
    {
      question: "What is required for a tree to be a Complete Binary Tree?",
      options: [
        "Every node must have exactly two children",
        "All levels must be completely full, no exceptions",
        "All levels are full except possibly the last, which fills left to right",
        "The tree must be sorted"
      ],
      correctAnswer: 2,
      explanation: "A Complete Binary Tree allows the last level to be partially filled, but only if it's filled left to right with no gaps."
    },
    {
      question: "Why do heaps rely on the Complete Binary Tree structure?",
      options: [
        "It makes every node a leaf",
        "It guarantees a compact, predictable height that keeps operations efficient",
        "It's required for sorting",
        "It doubles the number of comparisons needed"
      ],
      correctAnswer: 1,
      explanation: "The gap-free, left-packed shape of a Complete Binary Tree means it can be stored efficiently in an array and always has a minimal height for its node count."
    },
    {
      question: "What commonly causes a Binary Search Tree to become degenerate?",
      options: [
        "Inserting random values",
        "Inserting already-sorted data with no rebalancing",
        "Deleting the root node",
        "Using a heap instead"
      ],
      correctAnswer: 1,
      explanation: "Inserting sorted (or reverse-sorted) data into a plain BST with no rebalancing causes every new node to become the only child of the previous one, producing a skewed shape."
    }
  ];
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("the definition of a Full Binary Tree");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("degenerate/skewed tree behavior");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("Complete Binary Tree rules");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("why heaps use complete trees");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("what causes a BST to degenerate");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered binary tree types!";
  };
