...
const BinaryTreePropertiesQuiz = () => {
  const questions = [
    {
      question: "What is the height of a tree that consists of a single root node with no children?",
      options: ["-1", "0", "1", "Undefined"],
      correctAnswer: 1,
      explanation: "Height is measured in edges on the longest root-to-leaf path. A single node has no edges below it, so its height is 0."
    },
    {
      question: "What is the maximum number of nodes possible at depth d in a binary tree?",
      options: ["d", "2d", "2^d", "2^(d+1) - 1"],
      correctAnswer: 2,
      explanation: "Each level can have at most double the nodes of the level above it, giving 2^d as the maximum for depth d."
    },
    {
      question: "A binary tree with height h can have at most how many total nodes?",
      options: ["h + 1", "2h", "2^h", "2^(h+1) - 1"],
      correctAnswer: 3,
      explanation: "Summing 2^0 + 2^1 + ... + 2^h (the max nodes at every level from 0 to h) gives 2^(h+1) - 1."
    },
    {
      question: "What distinguishes a leaf node from an internal node?",
      options: [
        "A leaf has exactly one child",
        "A leaf has no children",
        "A leaf is always the root",
    ...
    },
    {
      ...
      options: [
        ...
        "Its height approaches n, making operations O(n) instead of O(log n)",
        "It can't store duplicate values",
        "It requires more comparisons per node"
      ],
      correctAnswer: 1,
      explanation: "Since most operations run in time proportional to height, a skewed tree's height of ~n turns what should be O(log n) operations into O(n), same as a linked list."
    }
  ];
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("understanding height for a single node");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("max nodes per depth level");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("max total node count formulas");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("leaf vs internal node terminology");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("why tree balance affects performance");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered binary tree properties!";
  };
