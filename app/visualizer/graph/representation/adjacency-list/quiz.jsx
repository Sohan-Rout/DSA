...
const AdjacencyListQuiz = () => {
  const questions = [
    {
      question: "What does an adjacency list store for each vertex?",
      options: [
        "A row of V values, one per possible neighbor, mostly zero",
        "Only the vertices it's actually connected to",
    ...
    },
    {
      ...
      options: [
        "Only u's list gets v appended",
        "Both u's list gets v appended and v's list gets u appended, since the edge goes both ways",
        "Nothing changes until the edge is looked up",
        "A new vertex is created"
      ],
      correctAnswer: 1,
      explanation: "An undirected edge has no single direction, so both endpoints need the other recorded as a neighbor."
    },
    {
    ...
    },
    {
      ...
      options: [
        "Those algorithms can only work with lists, never matrices",
        "Those algorithms repeatedly need each vertex's neighbors, which a list returns directly without scanning unrelated vertices",
        "Lists are always faster to build than matrices",
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("what an adjacency list stores");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("how undirected edges are mirrored");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("space complexity of a list");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("edge-existence check time");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("why lists suit neighbor-focused algorithms");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered Adjacency Lists!";
  };
