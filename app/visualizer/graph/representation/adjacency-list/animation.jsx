...
import { Plus, Shuffle, RotateCcw, ArrowRight } from "lucide-react";
...
const AdjacencyListVisualizer = () => {
  ...
  const addEdge = () => {
    ...
    if (chips.length > 0) {
      gsap.fromTo(chips, { scale: 1 }, { scale: 1.15, duration: 0.2, yoyo: true, repeat: 1 });
    }
  };
  ...
  const handleChipClick = (from, to) => {
    if (selectedEdge && selectedEdge.from === from && selectedEdge.to === to) {
      setSelectedEdge(null);
    ...
    setRecentEdge(null);
    setMessage(`Highlighting edge ${from} → ${to}`);
  };
  ...
  const isEdgeHighlighted = (from, to) => {
    ...
    if (selectedEdge && ((selectedEdge.from === from && selectedEdge.to === to) || (selectedEdge.from === to && selectedEdge.to === from))) return "selected";
    return null;
  };
    ...
    <main className="container mx-auto px-2 sm:px-6 pb-4">
      ...
      <div className="max-w-4xl mx-auto">
          ...
          <div className="min-h-72 flex justify-center overflow-auto py-4 rounded-lg bg-[radial-gradient(circle,var(--color-gray-200)_1px,transparent_1px)] dark:bg-[radial-gradient(circle,var(--color-neutral-800)_1px,transparent_1px)] bg-size-[16px_16px]">
            {vertices.length > 0 ? (
              <svg width="440" height="340" viewBox="0 0 440 340" className="mx-auto">
                <defs>
                  <linearGradient id="al-node-grad" x1="0" y1="0" x2="0" y2="1">
                  ...
                  return (
                    <g key={i}>
                      <line
                        ...
                        x2={b.x}
                        y2={b.y}
                        stroke={status === "recent" ? "#f59e0b" : status === "selected" ? "#8b5cf6" : "#818cf8"}
                        strokeWidth={status ? 3 : 2}
                        className={status ? "" : "dark:stroke-indigo-500/70"}
                        markerEnd={e.directed ? "url(#al-arrow)" : undefined}
                      />
                      {e.weight !== 1 && (
                        <>
                ...
                {positions.map((p, i) => (
                  <g key={i} ref={animateDropIn}>
                    <circle cx={p.x} cy={p.y} r="20" fill="url(#al-node-grad)" stroke="#1d4ed8" strokeWidth="1.5" />
        ...
        <div className="bg-white dark:bg-neutral-950 p-6 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-semibold mb-4">Adjacency List</h2>
          {vertices.length > 0 ? (
            <div className="space-y-3">
              {vertices.map((v) => (
                <div key={v} className="flex flex-wrap items-center gap-2">
                  <div className="flex items-center justify-center w-9 h-9 rounded-lg bg-blue-600 text-white text-sm font-bold shrink-0">
                    {v}
                  </div>
                  <ArrowRight size={14} className="text-gray-400 shrink-0" />
                  {adjList[v].length > 0 ? (
                    <div className="flex flex-wrap gap-1.5">
                      {adjList[v].map((entry, i) => {
                        const status = isEdgeHighlighted(v, entry.to);
                        return (
                          <button
                            key={i}
                            onClick={() => handleChipClick(v, entry.to)}
                            className={`list-chip-active px-2.5 py-1.5 rounded-md border text-xs font-medium transition-all duration-200 ${
                              status === "recent"
                                ? "bg-amber-300 dark:bg-amber-600 border-amber-500 dark:border-amber-400"
                                : status === "selected"
                                ? "bg-violet-300 dark:bg-violet-700 border-violet-500 dark:border-violet-400"
                                : "bg-gray-100 dark:bg-neutral-800 border-gray-300 dark:border-gray-700 hover:bg-gray-200 dark:hover:bg-neutral-700"
                            }`}
                          >
                            {entry.to}{entry.weight !== 1 ? ` (${entry.weight})` : ""}
                          </button>
                        );
                      })}
                    </div>
                  ) : (
                    <span className="text-xs text-gray-400 dark:text-gray-500 italic">no neighbors</span>
                  )}
                </div>
              ))}
            </div>
          ) : (
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">Add vertices to see the list</div>
          )}

          <div className="mt-4 flex flex-wrap items-center justify-center gap-3 text-xs text-gray-500 dark:text-gray-400">
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-sm bg-gray-100 dark:bg-neutral-800 inline-block border border-gray-300 dark:border-gray-700"></span>
              Neighbor entry
            </span>
            <span className="flex items-center gap-1.5">
