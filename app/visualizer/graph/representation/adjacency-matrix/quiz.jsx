...
const AdjacencyMatrixQuiz = () => {
  const questions = [
    {
      question: "What does cell (i, j) in an adjacency matrix represent?",
      options: [
        "The distance between vertex i and the origin",
        "Whether (or with what weight) there is an edge from vertex i to vertex j",
    ...
    },
    {
      ...
      options: [
        "It isn't always symmetric",
        "Every edge sets both cell (i, j) and cell (j, i) to the same value, since the edge goes both ways",
        "Symmetry is enforced by sorting the vertices alphabetically",
        "Only diagonal cells are ever set in undirected graphs"
      ],
      correctAnswer: 1,
      explanation: "An undirected edge between i and j has no direction, so both matrix entries representing it must match."
    },
    {
      question: "What is the time complexity of checking whether an edge exists between two specific vertices?",
      options: ["O(V)", "O(E)", "O(1)", "O(V²)"],
      correctAnswer: 2,
      explanation: "Checking a specific cell is a direct array index lookup, independent of how large or dense the graph is."
    },
    {
    ...
    },
    {
      ...
      correctAnswer: 1,
      explanation: "The V×V grid is allocated up front no matter how many edges actually exist, so a sparse graph pays for a huge number of cells that just hold zero."
    }
  ];
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("what a matrix cell represents");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("why undirected graphs give symmetric matrices");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("edge-existence lookup time");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("neighbor iteration time");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("why matrices waste space on sparse graphs");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered Adjacency Matrices!";
  };
