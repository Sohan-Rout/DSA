...
const DfsQuiz = () => {
  const questions = [
    {
      question: "How does DFS explore a graph from a start vertex?",
      options: [
        "It visits every vertex at distance 1 before any vertex at distance 2",
        "It plunges as deep as possible down one path, only backtracking once that path is exhausted",
        "It visits vertices in alphabetical order",
        "It visits every vertex simultaneously"
      ],
      correctAnswer: 1,
      explanation: "DFS commits to going deeper along one branch before trying any other branch, unlike BFS's ring-by-ring expansion."
    },
    {
      ...
      correctAnswer: 1,
      explanation: "A stack's LIFO order is what makes DFS dive deeper: the newest discovery jumps ahead of anything found earlier and still waiting."
    },
    {
      ...
      correctAnswer: 1,
      explanation: "Because a vertex can be pushed more than once (by different discoverers) before it's processed, the visited check happens at pop time, and any later pop of the same already-visited vertex is simply skipped."
    },
    {
      ...
      correctAnswer: 1,
      explanation: "A recursive DFS function implicitly uses the program's call stack, producing the same LIFO exploration order as an explicit stack would."
    },
    {
      question: "Which of these tasks is DFS especially well suited for?",
      options: [
        "Finding the shortest path by number of edges",
        "Detecting cycles and performing topological sorts on a dependency graph",
        "Finding the vertex closest to the start in Euclidean distance",
