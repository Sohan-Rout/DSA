...
const KruskalQuiz = () => {
  const questions = [
    {
      ...
      options: [
        ...
        "A cycle-free subset of edges connecting every vertex, with the smallest possible total edge weight",
        "A tree containing only the heaviest edges",
        "Any subgraph with exactly one vertex"
      ],
      correctAnswer: 1,
      explanation: "\"Spanning\" means every vertex is reachable, \"tree\" means no cycles, and \"minimum\" means no cheaper such structure exists."
    },
    {
      question: "In what order does Kruskal's algorithm process edges?",
      options: [
        "In the order they were added to the graph",
        "Sorted by weight, from cheapest to most expensive",
        "Sorted by weight, from most expensive to cheapest",
        "In a random order"
      ],
      correctAnswer: 1,
      explanation: "Processing cheapest-first is exactly what makes the greedy accept-if-no-cycle rule produce a minimum-weight result."
    },
    {
      ...
      correctAnswer: 1,
      explanation: "If both endpoints are already connected through previously accepted edges, adding this edge too would close a cycle, which a tree can't contain."
    },
    {
      question: "What data structure efficiently answers \"are these two vertices already connected?\" during Kruskal's algorithm?",
      options: [
        "A binary search tree",
        "A Union-Find (disjoint-set) structure",
        "A hash map of adjacency lists",
        "A priority queue"
      ],
      correctAnswer: 1,
      explanation: "Union-Find tracks which component each vertex belongs to, answering \"same component?\" (find) and merging components (union) in close to constant time."
    },
    {
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("the definition of a minimum spanning tree");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("the edge processing order");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("when an edge gets rejected");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("the role of Union-Find");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("what dominates the time complexity");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered Kruskal's Algorithm!";
  };
