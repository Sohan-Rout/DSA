...
const DijkstraQuiz = () => {
  const questions = [
    {
      ...
      options: [
        "The fewest number of edges between two vertices",
        "The shortest total-weight path from a start vertex to every other vertex",
    ...
    },
    {
      ...
      options: [
        ...
        "All weights must be whole numbers",
    ...
    },
    {
      ...
      options: [
        "The alphabetically first unvisited vertex",
        "The unvisited vertex with the smallest current tentative distance",
        "A randomly chosen unvisited vertex",
        "The vertex with the most edges"
      ],
      correctAnswer: 1,
      explanation: "Always finalizing the closest unvisited vertex is what guarantees its distance is already correct and will never need revising."
    },
    {
      question: "What does \"relaxing\" an edge (u, v) mean?",
      options: [
        "Removing the edge from the graph",
        "Checking whether going through u produces a shorter distance to v than currently recorded, and updating it if so",
        "Doubling the edge's weight",
        "Marking both u and v as visited"
      ],
      correctAnswer: 1,
      explanation: "Relaxation is the update step: if dist[u] + weight(u,v) is less than the current dist[v], dist[v] gets improved."
    },
    {
      ...
      correctAnswer: 2,
      explanation: "Each vertex is extracted from the priority queue once and each edge triggers at most one relaxation, both costing O(log V) with a binary heap."
    }
  ];
  ...
  const calculateWeakAreas = () => {
    ...
    if (answers[0] !== questions[0].correctAnswer) {
      weakAreas.push("what Dijkstra's algorithm computes");
    }
    if (answers[1] !== questions[1].correctAnswer) {
      weakAreas.push("the non-negative weight requirement");
    }
    if (answers[2] !== questions[2].correctAnswer) {
      weakAreas.push("which vertex gets finalized next");
    }
    if (answers[3] !== questions[3].correctAnswer) {
      weakAreas.push("what edge relaxation means");
    }
    if (answers[4] !== questions[4].correctAnswer) {
      weakAreas.push("time complexity with a priority queue");
    }

    return weakAreas.length > 0
      ? `Focus on improving: ${weakAreas.join(', ')}. Review the corresponding sections above.`
      : "Perfect! You've mastered Dijkstra's Algorithm!";
  };
