...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden mt-16 w-full bg-white dark:bg-black transition-all duration-300 ease-in-out overflow-hidden ${
          isMenuOpen ? 'max-h-96' : 'max-h-0'
        }`}
      >
        <div className="flex flex-col px-6 py-4 space-y-4">
          {user ? (
            <p className="text-black dark:text-white font-medium px-4 py-3">
              Welcome, {user.username}
            </p>
          ) : (
            <button
